...
namespace ErpStudy.Application.UseCases.Products
{
    ...
    {
        public async Task<Result<Product>> ExecuteAsync(UpdateProductDTO request)
        {
            try
            {
                var validationResult = await new UpdateProductDTOValidator().ValidateAsync(request);
                ...
                var product = await productRepository.Update(newProduct);
